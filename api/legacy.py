...
(precede il flusso a job in background; mantenuta per compatibilità
con chiamanti storici).
"""
...
async def run_suite(
    ...
    if not tasks:
        ...
        raise HTTPException(
            status_code=404,
            detail=(
                "Nessuna suite di test configurata "
                "o eseguibile con i parametri passati."
            ),
        )

    logger.info(
        "Avvio esecuzione simultanea multi-stack. "
        "Task attivi in parallelo: %d",
        len(tasks),
    )

    liste_risultati = await asyncio.gather(
        *tasks
    )
    ...
    return {
        ...
        "dns4_server_utilizzato": (
            request.dns4_server
            if esegui_ipv4
            else None
        ),
        "dns6_server_utilizzato": (
            request.dns6_server
            if esegui_ipv6
            else None
        ),
